...
function resetDrawLoop(state, renderer, telop) {
  ...
  window.gDrawLoopID = setInterval(drawLoop, intervalTime,
  								state, renderer, telop);
}
...
function initTelopTypeDropdown(renderer, state) {
  ...
  telopTypes.forEach((item) => {
    ...
    a.html(item['name']);
    a.on('click', () => {
    	telopTypeDropdownClicked(item['name'], item['class'],
      													renderer, state);
    });
...
class BaseTelop {
  ...
  //テロップ領域の初期化を行う
  initTelop(state) { this.renderer.clearDOM(); }
  //テロップ個別のUIを設置する。parentはdivタグ
	initExtUI(parent) { this._clearExtUI(parent); }
...
`);
  ...
  retrieveExtUI() {
    ...
    this.bold = $('#input-bold-on').prop('class').split(/ /).includes('active');
  }
